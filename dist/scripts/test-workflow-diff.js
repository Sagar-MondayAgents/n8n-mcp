...
async function testNodeReferenceByIdAndName() {
    ...
    if (result.success) {
        const node = result.workflow.nodes.find((n) => n.id === 'webhook_1');
    ...
    if (result.success) {
        const node = result.workflow.nodes.find((n) => n.name === 'Webhook');
        console.log(`   - ✅ Success: path = ${node.parameters.path}`);
    }
    else {
        console.log(`   - ❌ Failed: ${result.errors[0].message}`);
...
//# sourceMappingURL=test-workflow-diff.js.map